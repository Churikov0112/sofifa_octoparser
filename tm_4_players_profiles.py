...
def get_player_profile_data(player_id, max_retries=5, retry_delay=3):
    """Получает данные профиля игрока из API с повторными попытками и обработкой 503"""
...
def extract_player_ids_from_players(players_data):
    """Извлекает все ID игроков из данных об игроках (используем id)"""
    player_ids = set()
...
def process_players_profiles(input_file, output_file, max_workers=2, delay=3.0):
    """Обрабатывает игроков и получает данные профилей с улучшенной обработкой 503"""
    ...
    report = {
        "processing_summary": {
            ...
            "api_endpoint": "https://transfermarkt-api.fly.dev/players/{id}/profile"
        },
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    }

    # Сохраняем отчет
    report_filename = "tm_players_profiles_report.json"
...
def main():
    ...
    output_filename = "tm_players_profiles.json"  # Выходной файл
    ...
    # Обрабатываем данные профилей с увеличенными задержками
    process_players_profiles(input_filename, output_filename, max_workers=1, delay=5.0)

    print("\n🎉 Все задачи завершены!")
    print(f"Данные сохранены в {output_filename}")
    print(f"Отчет сохранен в tm_players_profiles_report.json")
